...
def _handle_flowstats_received (event):
   global src_dpid,rec_time1,rec_time2,link1_bytes,link2_bytes
   received_time = time.time() - start_time
   for f in event.stats:
     if f.match.in_port==1 and f.match.dl_type==0x0806  and event.connection.dpid==src_dpid:
      # print "pkt",f.packet_count
       if f.byte_count>0:
         link1_bytes=f.byte_count
         print getTheTime(),"Bandwidth of Link between s1 and s2 is",(link1_bytes/1024)/received_time,"KB/sec"

     if f.match.in_port==2 and f.match.dl_type==0x0806 and event.connection.dpid==src_dpid:
       if f.byte_count>0:
         link2_bytes=f.byte_count  
         print getTheTime(),"Bandwidth of Link between s2 and s3 is ",(link2_bytes/1024)/received_time,"KB/sec"
